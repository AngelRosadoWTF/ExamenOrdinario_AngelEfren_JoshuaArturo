...
class Program
{
    ...
    public static void AdministracionDeMascotas()
    {
        ...
        static void BuscarMascotaPorEspecie()
        {
            ...
            var mascotasPorEspecie = personas.SelectMany(p => p.Mascotas).Where(m => m.Especie.Equals(especie, StringComparison.OrdinalIgnoreCase) && m.DisponibleParaAdopcion).ToList();
